...
const SLOT_H = 64; // px per hour  →  1 min = 64/60 px
...
export default function BookingsBoardPage() {
  ...
  // 7-day strip: centre around selected date (3 before, 3 after)
  ...
  /* ── Drag & Drop ────────────────────────────────────────────────── */
  const handleDragStart = (bookingId: string) => setDragging(bookingId);
  ...
  const handleDrop = async (e: React.DragEvent, staffId: string) => {
    ...
    const minute = Math.round((totalMin % 60) / 30) * 30;

    const newStart = new Date(date);
    newStart.setHours(Math.min(END_HOUR - 1, Math.max(START_HOUR, hour)), minute === 60 ? 0 : minute, 0, 0);
    if (minute === 60) newStart.setHours(newStart.getHours() + 1);
    ...
    <ProLayout title="Записи" actions={actions}>
      ...
      <div className={styles.dateStrip}>
        {stripDays.map(d => {
          ...
          return (
            <button
              key={dStr}
              className={`${styles.dayBtn} ${isSelected ? styles.dayBtnActive : ''} ${isTodayDay ? styles.dayBtnToday : ''}`}
      ...
      {staff.length > 1 && (
        <div className={styles.masterFilter}>
          <button className={`${styles.masterChip} ${selectedMasterId === null ? styles.masterChipActive : ''}`} onClick={() => setSelectedMasterId(null)}>
            Все
          </button>
          {staff.map(s => (
            <button
      ...
      <BottomSheet open={slotTarget !== null} onClose={() => setSlotTarget(null)} title="Новая запись">
        {slotTarget && (
          <div className={styles.bookingForm}>
            ...
            {/* Service picker — cards instead of select */}
        ...
        {selectedBooking && (
          <div className={styles.bookingDetail}>
            ...
            {selectedBooking.notes && (
              <div className={styles.detailNotes}>💬 {selectedBooking.notes}</div>
            )}
            ...
            <div className={styles.detailActions}>
              ...
              {selectedBooking.status === 'confirmed' && (
                <div className={styles.quickStatusRow}>
                  <button className={styles.arrivedBtn} disabled={actionLoading} onClick={() => handleBookingAction('completed')}>
                    ✓ Пришёл
                  </button>
                  <button className={styles.noShowActionBtn} disabled={actionLoading} onClick={() => handleBookingAction('no_show')}>
                    ✗ Не явился
                  </button>
                </div>
              )}
              {(selectedBooking.status === 'pending' || selectedBooking.status === 'confirmed') && (
                <button className={styles.cancelActionBtn} disabled={actionLoading} onClick={() => handleBookingAction('cancelled')}>
                  Отменить запись
                </button>
              )}
            </div>
...
function TimelineView({
  hours, staff, bookings, isToday,
  dragging, dragOverStaffId,
  onDragStart, onDragOverStaff, onDrop,
  ...
  // Auto-scroll to now on today
  useEffect(() => {
    if (!isToday || !scrollRef.current) return;
    const offset = Math.max(0, minToTop(nowMin) - 80);
  ...
  const handleColumnClick = (e: React.MouseEvent, staffId: string, staffName: string) => {
    ...
    const minute = Math.round((totalMin % 60) / 30) * 30 === 60 ? 0 : Math.round((totalMin % 60) / 30) * 30;
    ...
    <div className={styles.timeline}>
      ...
      <div className={styles.timelineBody} ref={scrollRef}>
        ...
        <div className={styles.hoursCol}>
          {hours.map(h => (
            <div key={h} className={styles.hourMark}>
        ...
        <div className={styles.columnsArea} style={{ gridTemplateColumns: `repeat(${colCount}, minmax(72px, 1fr))` }}>
          {staff.map(s => {
            ...
            const isDragOver = dragOverStaffId === s.id;
            return (
              <div
                key={s.id}
                className={`${styles.staffCol} ${isDragOver ? styles.staffColDragOver : ''}`}
...
function BookingCard({ booking, isDragging, onDragStart, onClick }: BookingCardProps) {
  ...
  return (
    <div
      className={`${styles.bookingCard} ${statusClass} ${isReschedule ? styles.bookingCardReschedule : ''} ${isDragging ? styles.bookingCardDragging : ''}`}
    ...
    >
      ...
      {!isCompact && (
        <span className={styles.cardService}>
          {isReschedule ? '↻ Перенос' : (booking.services?.name ?? '—')}
        </span>
      )}
    </div>
  );
...
/* ── List view ─────────────────────────────────────────────────────────────── */
function ListView({ bookings, staff, onBookingClick }: {
  bookings: Booking[]; staff: Master[]; onBookingClick: (b: Booking) => void;
}) {
        ...
        return (
          ...
          >
            ...
            <div className={styles.agendaInfo}>
              ...
              <span className={styles.agendaService}>
                {isReschedule ? '↻ Запрос на перенос' : (b.services?.name ?? '—')}
              </span>
